...
impl Middleware for LoggingMiddleware {
    async fn before_dispatch(
        ...
        let action = transaction.get_meta::<String>("action")
            .map(|s| s.as_str())
            .unwrap_or("unknown");
            
        println!("🔍 [{}] 事务处理开始 - ID: {}, 动作: {}", 
    ...
    async fn after_dispatch(
        ...
        for transaction in transactions {
            ...
            let start_time = transaction.get_meta::<SystemTime>("middleware_start_time");
            ...
            println!("✅ [{}] 事务处理完成 - ID: {}, 动作: {}{}", 
                     self.name, transaction.id, action, duration_info);
        }
        
        if let Some(state) = state {
            println!("📊 [{}] 当前状态版本: {}, 插件数量: {}", 
                     self.name, state.version, state.plugins().len());
        }
...
impl Middleware for MetricsMiddleware {
    async fn before_dispatch(
        ...
        let count = self.transaction_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed) + 1;
        println!("📈 [{}] 开始性能监控 - 事务 #{}, 步骤数: {}", 
                 self.name, count, transaction.steps.len());
    ...
    async fn after_dispatch(
        ...
        for transaction in transactions {
            if let Some(start_time) = transaction.get_meta::<Instant>("metrics_start_time") {
                ...
                if let Some(state) = &state {
                    ...
                    println!("   - 字段实例数: {}", state.fields_instances.len());
                }
                
                // 性能警告
                if duration.as_millis() > 100 {
                    println!("⚠️  [{}] 性能警告: 事务处理时间过长 ({:?})", self.name, duration);
                }
                
                if steps_count > 10 {
                    println!("⚠️  [{}] 复杂度警告: 事务步骤数量较多 ({})", self.name, steps_count);
                }
            }
        }
...
impl ValidationMiddleware {
    pub fn new() -> Self {
        Self {
            name: "ValidationMiddleware".to_string(),
        }
    }
    
    fn validate_transaction_basic(&self, transaction: &Transaction) -> Result<(), String> {
    ...
    fn validate_transaction_post(&self, transaction: &Transaction) -> Result<(), String> {
    ...
    async fn after_dispatch(
        ...
        if validation_needed {
            if let Some(state) = state {
                ...
                println!("📋 [{}] 生成后置验证事务: {}", self.name, validation_tr.id);
                
                return Ok(MiddlewareResult::with_transactions(Ok(()), Some(validation_tr)));
            }
        }
