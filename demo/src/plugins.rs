                    ...
                    "user_login" => {
                        if let Some(username) = tr.get_meta::<String>("username") {
                            if let Some(role) = tr.get_meta::<String>("role") {
                                println!("   🔑 用户管理插件: 处理用户登录 - {} ({})", username.clone(), role);
                                // 生成用户状态更新事务
                                ...
                                new_tr.set_meta("username", username.as_ptr().clone());
                ...
                match action.as_str() {
                    ...
                    "update_user_status" => {
                        if let Some(username) = tr.get_meta::<String>("username") {
                            ...
                            println!("   🔄 更新用户状态: {}", username.as_str());
                        }
                    }
                    _ => {}
                }
        ...
        for tr in transactions {
            if let Some(action) = tr.get_meta::<String>("action") {
                match action.as_str() {
                    "user_login" => {
                        if let Some(username) = tr.get_meta::<String>("username") {
                            println!("   🛡️ 权限验证插件: 验证用户 {} 的登录权限", username);
                        }
                    }
                    "create_document" => {
                        if let Some(title) = tr.get_meta::<String>("title") {
                            ...
                            new_tr.set_meta("document_title", title.as_ptr().clone());
                            return Ok(Some(new_tr));
                        }
                    }
                    "add_heading" | "add_paragraph" | "add_list" | "add_table" => {
                        ...
                        new_tr.set_meta("content_type", action.as_ptr().clone());
                        return Ok(Some(new_tr));
                    }
                    "document_edit" => {
            ...
            match action.as_str() {
                "document_edit" => {
                    ...
                    if let Some(user_id) = transaction.get_meta::<String>("user_id") {
                        println!("   🔍 权限验证插件: 验证用户 {} 的编辑权限", user_id);
            ...
            if let Some(action) = tr.get_meta::<String>("action") {
                match action.as_str() {
                    "permission_validated" => {
                        new_state.last_check_time = std::time::SystemTime::now();
                    }
                    _ => {}
                }
            }
        ...
        for tr in transactions {
            if let Some(action) = tr.get_meta::<String>("action") {
                match action.as_str() {
                    "user_login" => {
                        if let Some(username) = tr.get_meta::<String>("username") {
                            println!("   📝 审计日志插件: 记录用户登录事件 - {}", username);
                        }
                    }
                    "create_document" => {
                        if let Some(title) = tr.get_meta::<String>("title") {
                            println!("   📄 审计日志插件: 记录文档创建 - {}", title);
                        }
                    }
                    "add_heading" => {
                        if let Some(text) = tr.get_meta::<String>("text") {
                            if let Some(level) = tr.get_meta::<u32>("level") {
                                println!("   📋 审计日志插件: 记录标题添加 - H{}: {}", level, text);
                            }
                        }
                    }
                    "add_paragraph" => {
        ...
        for tr in transactions {
            if let Some(action) = tr.get_meta::<String>("action") {
                match action.as_str() {
                    "user_login" => {
                        if let Some(username) = tr.get_meta::<String>("username") {
                            println!("   💾 缓存管理插件: 缓存用户会话 - {}", username);
                        }
                    }
                    "create_document" => {
                        if let Some(title) = tr.get_meta::<String>("title") {
                            println!("   📄 缓存管理插件: 缓存文档元数据 - {}", title);
                            // 生成文档缓存事务
                            ...
                            cache_tr.set_meta("document_title", title.as_ptr().clone());
                            return Ok(Some(cache_tr));
                        }
                    }
                    "add_heading" | "add_paragraph" | "add_list" | "add_table" => {
                        println!("   🔄 缓存管理插件: 更新内容缓存 - {}", action);
                        // 生成内容缓存更新事务
                        let mut cache_tr = Transaction::new(new_state);
                        cache_tr.set_meta("generated_by", "cache_plugin");
                        cache_tr.set_meta("action", "content_cache_updated");
                        cache_tr.set_meta("content_type", action.as_ptr().clone());
                        return Ok(Some(cache_tr));
                    }
                    "create_snapshot" => {
