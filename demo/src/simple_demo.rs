...
impl Command for SimpleCommand {
    async fn execute(&self, tr: &mut moduforge_state::transaction::Transaction) -> moduforge_transform::TransformResult<()> {
...
pub async fn run_simple_demo() -> Result<()> {
    ...
    editor.command(Arc::new(login_cmd)).await
    ...
    // 文档创建流程
    println!("\n📄 文档管理流程:");
    let create_doc_cmd = CreateDocumentCommand::new("协作文档示例", "展示多插件协作的示例文档");
    editor.command(Arc::new(create_doc_cmd)).await
    ...
    editor.command(Arc::new(add_heading_cmd)).await
        .map_err(|e| anyhow::anyhow!("添加标题失败: {}", e))?;
    
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    
    let add_para_cmd = AddParagraphCommand::new("ModuForge-RS 是一个基于 Rust 的现代化文档编辑框架，具有强大的插件系统、中间件支持和事务化状态管理。");
    editor.command(Arc::new(add_para_cmd)).await
    ...
    editor.command(Arc::new(add_list_cmd)).await
    ...
    let add_table_cmd = AddTableCommand::new(
        ...
        vec![
            vec!["用户管理".to_string(), "✅ 活跃".to_string(), "高".to_string()],
            vec!["权限控制".to_string(), "✅ 活跃".to_string(), "高".to_string()],
            vec!["审计日志".to_string(), "✅ 活跃".to_string(), "中".to_string()],
            vec!["缓存管理".to_string(), "✅ 活跃".to_string(), "低".to_string()],
        ]
    );
    editor.command(Arc::new(add_table_cmd)).await
    ...
    editor.command(Arc::new(snapshot_cmd)).await
    ...
    editor.command(Arc::new(sync_cmd)).await
    ...
    editor.command(Arc::new(validate_cmd)).await
