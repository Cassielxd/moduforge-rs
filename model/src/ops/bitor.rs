...
use crate::{
    error::PoolResult, id_generator::IdGenerator, mark::Mark, node::Node, types::NodeId
};
...
impl<'a> BitOr<NodeId> for NodeRef<'a> {
    ...
    fn bitor(
        ...
        // 获取另一个节点的所有子节点
        let other_children = self.tree.children(&other_node_id).unwrap_or_default();
...
impl<'a> BitOr<Mark> for MarkRef<'a> {
    ...
    fn bitor(
        ...
        // 检查标记是否已存在
        let existing_marks = self.tree.get_marks(&self.key.clone().into()).unwrap_or_default();
        let mark_exists = existing_marks.iter().any(|existing_mark| {
            existing_mark.r#type == mark.r#type && existing_mark.attrs == mark.attrs
        });
