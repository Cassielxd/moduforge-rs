...
impl AttributeParser {
    ...
    /// - 多个 attr 属性会返回错误
    /// - 无效的 JSON 格式会返回错误
    fn parse_field_attr_attribute(field: &Field) -> MacroResult<(bool, Option<DefaultValue>)> {
        use syn::{Meta};
            ...
            if let Some(ident) = attr.path().get_ident() {
                if ident == "attr" {
                    ...
                    match &attr.meta {
                        ...
                        Meta::List(meta_list) => {
                            // 解析参数列表
                            default_value = Self::parse_attr_meta_list(meta_list, field)?;
                        }
    ...
    /// - **单一职责**: 只负责参数列表解析
    /// - **接口隔离**: 提供专门的参数解析接口
    fn parse_attr_meta_list(meta_list: &syn::MetaList, field: &Field) -> MacroResult<Option<DefaultValue>> {
                ...
                Meta::NameValue(name_value) => {
                    if let Some(ident) = name_value.path.get_ident() {
                        if ident == "default" {
                            ...
                            // 解析默认值
                            let value_str = Self::extract_value_from_expr(&name_value.value)?;
        ...
        match expr {
            ...
            syn::Expr::Lit(expr_lit) => {
                match &expr_lit.lit {
                    ...
                    Lit::Int(lit_int) => Ok(lit_int.base10_digits().to_string()),
                    Lit::Float(lit_float) => Ok(lit_float.base10_digits().to_string()),
            ...
            syn::Expr::Unary(expr_unary) => {
                if matches!(expr_unary.op, syn::UnOp::Neg(_)) {
                    let inner = Self::extract_value_from_expr(&expr_unary.expr)?;
                    Ok(format!("-{}", inner))
                } else {
                    ...
                    syn::Fields::Named(named_fields) => {
                        ...
                        for field in &named_fields.named {
                            if let Some(field_name) = &field.ident {
                                // 解析字段的 attr 属性（可能包含默认值）
                                let (is_attr, default_value) = Self::parse_field_attr_attribute(field)?;
...
mod tests {
    ...
    fn test_field_config_default_value_methods() {
        use syn::parse_quote;
        use crate::parser::default_value::{DefaultValue, DefaultValueType, DefaultValueParser};
        ...
        // 创建一个默认值
        let default_value = DefaultValueParser::parse("hello world", None).unwrap();
        ...
        assert!(matches!(stored_value.value_type, DefaultValueType::String(ref s) if s == "hello world"));
        ...
        assert_eq!(field_config2.get_default_value().unwrap().raw_value, "hello world");
    }
    ...
    fn test_parse_field_attr_with_default_values() {
        ...
        let (is_attr, default_value) = AttributeParser::parse_field_attr_attribute(&field).unwrap();
        assert!(is_attr);
        ...
        assert!(matches!(default_val.value_type, DefaultValueType::String(ref s) if s == "hello world"));
        ...
        assert!(matches!(default_val.value_type, DefaultValueType::Integer(42)));
    ...
    fn test_complete_field_parsing_with_defaults() {
        ...
        // 验证各个字段的默认值设置
        let simple_field = config.attr_fields.iter()
            .find(|f| f.name == "simple_field")
            .expect("应该找到 simple_field");
        assert!(!simple_field.has_default_value());
        
        let field_with_default = config.attr_fields.iter()
            .find(|f| f.name == "field_with_default")
            .expect("应该找到 field_with_default");
        assert!(field_with_default.has_default_value());
        assert_eq!(field_with_default.get_default_value().unwrap().raw_value, "default value");
        
        let numeric_field = config.attr_fields.iter()
        ...
        let boolean_field = config.attr_fields.iter()
            .find(|f| f.name == "boolean_field")
            .expect("应该找到 boolean_field");
        assert!(boolean_field.has_default_value());
        assert_eq!(boolean_field.get_default_value().unwrap().raw_value, "true");
    }
}