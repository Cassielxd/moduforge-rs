...
use moduforge_collaboration::{Result, SyncService, YrsMiddleware, YrsManager, WebSocketServer};
...
async fn test_collaboration() -> Result<()> {
    ...
    let sync_service = Arc::new(SyncService::new(yrs_manager, ws_server.clone()));

    // 2. 创建并配置 ForgeRuntime (Editor)
    let room_id = "demo-room".to_string();
    let runtime = Arc::new(Mutex::new(build_runtime(sync_service.clone(), room_id.clone()).await));
    
    // 4. 启动 WebSocket 服务器
    let ws_addr: SocketAddr = "127.0.0.1:8080".parse().unwrap();
    let server_future = ws_server.start(ws_addr, sync_service.clone(), runtime.clone());
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match runtime_clone_for_commands.lock().await.command(Arc::new(TestCommand)).await {
    ...
    tokio::select! {
        ...
        res = server_future => {
            if let Err(e) = res {
                tracing::error!("🔌 WebSocket 服务器因错误而关闭: {}", e);
            } else {
                tracing::info!("🔌 WebSocket 服务器已关闭");
            }
        }
    }
    Ok(())
...
async fn build_runtime(sync_service: Arc<SyncService>, room_id: String) -> ForgeRuntime {
    let nodes = nodes();
    ...
    for node in nodes {
     ...
     middleware_stack.add(YrsMiddleware {
