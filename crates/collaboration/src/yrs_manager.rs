...
impl YrsManager {
    ...
    /// 获取或创建房间的 Yrs 文档
    pub fn get_or_create_doc(&self, room_id: &str) -> Arc<Doc> {
    ...
    /// 获取房间的Yrs文档（用于直接操作）
    pub fn get_doc(&self, room_id: &str) -> Option<Arc<Doc>> {
        self.rooms.get(room_id).map(|entry| entry.value().clone())
    }
    
    /// 移除房间的Doc
    pub fn remove_doc(&self, room_id: &str) -> Option<(String, Arc<Doc>)> {
    ...
    pub fn get_full_state_update(&self, room_id: &str) -> Result<Vec<u8>> {
        let doc = self.get_doc(room_id).ok_or_else(|| TransmissionError::RoomNotFound(room_id.to_string()))?;
