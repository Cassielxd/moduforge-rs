...
pub enum BroadcastMessage {
    /// 向房间广播文本消息
    ToRoom { room_id: String, message: Arc<String>, exclude_client: Option<String> },
    /// 向房间广播二进制消息
    ToRoomBinary { room_id: String, data: Arc<Vec<u8>>, exclude_client: Option<String> },
    /// 向特定客户端发送消息
...
impl WebSocketServer {
    ...
    pub async fn start(
        ...
        runtime: Arc<Mutex<ForgeRuntime>>
    ) -> Result<()> {
    ...
    async fn handle_connection(
        ...
        let send_task = tokio::spawn(async move {
            while let Some(message) = msg_rx.recv().await {
                if ws_sender.send(message.into()).await.is_err() {
                    break;
                }
            }
        });
        ...
        while let Some(msg) = ws_receiver.next().await {
            match msg {
                    ...
                    if let Ok(ws_msg) = serde_json::from_str::<WsMessage>(&text) {
                        if let Err(e) = self.handle_message(
                            ...
                            if let Ok(json) = serde_json::to_string(&error_msg) {
                                let _ = self.broadcast_tx.send(BroadcastMessage::ToClient {
                ...
                Ok(Message::Binary(_data)) => {
                    // 二进制消息现在应该通过YrsUpdate JSON消息包装
                    tracing::warn!("收到未包装的二进制消息，已忽略");
                }
                Ok(Message::Close(_)) => {
                    break;
                }
        ...
        // 清理连接
        send_task.abort();
        self.cleanup_client(&client_id_clone).await;
    ...
    async fn handle_message(
        ...
        match message {
            WsMessage::JoinRoom { room_id } => {
                ...
                self.room_clients.entry(room_id.clone()).or_default().push(client_id.to_string());
                let _doc = self.yrs_manager.get_or_create_doc(&room_id);
                tracing::info!("客户端 {} 加入房间 {}", client_id, room_id);
                Ok(())
            }
            WsMessage::LeaveRoom { room_id } => {
                if let Some(mut client_list) = self.room_clients.get_mut(&room_id) {
            ...
            WsMessage::YrsUpdate { room_id, update } => {
                let doc = self.yrs_manager.get_doc(&room_id).ok_or_else(|| TransmissionError::RoomNotFound(room_id.clone()))?;
                if let Err(e) = apply_update_to_doc(&doc, &update) {
                    ...
                    }).map_err(|e| TransmissionError::Other(anyhow::anyhow!(e.to_string())))?;
                }
                Ok(())
            }
            WsMessage::YrsSyncRequest { room_id, state_vector } => {
                tracing::debug!("收到来自 {} 的同步请求，房间: {}", client_id, room_id);
                
                let tree = runtime.lock().await.doc().get_inner().clone();
                let diff_update = sync_service.handle_sync_request(&room_id, &tree, &state_vector).await?;
    ...
    async fn cleanup_client(
        ...
        if let Some((_, client)) = self.clients.remove(client_id) {
            ...
            if !room_id.is_empty() {
                let should_remove_doc = if let Some(mut room) = self.room_clients.get_mut(&room_id) {
...
fn apply_update_to_doc(doc: &Arc<yrs::Doc>, update: &[u8]) -> crate::Result<()> {
...
#[allow(dead_code)]
fn get_diff_update_from_doc(doc: &Arc<yrs::Doc>, sv: &[u8]) -> std::result::Result<Vec<u8>, yrs::encoding::read::Error> {
