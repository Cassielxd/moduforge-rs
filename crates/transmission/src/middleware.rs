...
impl Middleware for YrsMiddleware {
    ...
    async fn after_dispatch(
        ...
        tracing::info!("同步事务: {} 个到房间 {}", transactions.len(), self.room_id);
        
        let state = match state {
            Some(s) => s,
            None => {
                error!("YrsMiddleware after_dispatch called without state, cannot sync.");
                return Ok(None);
            }
        };
        
        // 使用新的批量二进制格式同步
        if let Err(e) = self.sync_service.handle_transaction_applied(&self.room_id, transactions, &state, None).await {
            error!("YRS同步失败: {}", e);
        }
