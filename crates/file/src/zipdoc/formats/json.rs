...
pub fn read_and_decode_snapshot_shards_json<
    R: Read + Seek,
    T: DeserializeOwned,
>(
    ...
    for raw in shards_raw.iter() {
        let val: T = serde_json::from_slice(raw).map_err(io::Error::other)?;
        out.push(val);
    }
    Ok((meta, out))
}
...
pub fn write_parent_map_json<W, T>(
    zw: &mut ZipDocumentWriter<W>,
    parent_map: &T,
    zstd_level: i32,
) -> io::Result<()>
where
    ...
    T: Serialize,
{
    let bytes = serde_json::to_vec(parent_map).map_err(io::Error::other)?;
    let zst = zstd::stream::encode_all(&bytes[..], zstd_level)
        .map_err(io::Error::other)?;
    zw.add_stored("snapshot/parent_map.json.zst", &zst)
}

pub fn read_parent_map_json<R, T>(
    zr: &mut ZipDocumentReader<R>
) -> io::Result<T>
where
    R: Read + Seek,
    T: DeserializeOwned,
{
    let zst = zr.read_all("snapshot/parent_map.json.zst")?;
