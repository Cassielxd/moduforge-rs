    ...
    T: Serialize,
{
    ...
    for i in 0..meta.num_shards {
        ...
        let bytes = serde_cbor::to_vec(&v).map_err(io::Error::other)?;
...
>(
    ...
    for raw in shards_raw.iter() {
        let val: T = serde_cbor::from_slice(raw).map_err(io::Error::other)?;
        out.push(val);
    }
    Ok((meta, out))
}
...
where
    ...
    T: Serialize,
{
    let bytes = serde_cbor::to_vec(parent_map).map_err(io::Error::other)?;
    let zst = zstd::stream::encode_all(&bytes[..], zstd_level)
        .map_err(io::Error::other)?;
