...
fn demo_smart_reading() -> Result<()> {
    ...
    println!("   流式处理: {}", if config.enable_streaming { "启用" } else { "禁用" });
    println!();

    let files = ["tiny.txt", "small.bin", "medium.data", "large.blob"];
    
    println!("📊 智能读取演示:");
    println!("{:<12} {:<10} {:<8} {:<12} {:<15} {:<10}", 
...
fn demonstrate_smart_reading(reader: &mut ZipDocumentReader<File>, filename: &str) -> Result<()> {
    // 获取文件信息和推荐策略
