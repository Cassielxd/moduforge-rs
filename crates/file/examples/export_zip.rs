...
mod tests {
    ...
    async fn roundtrip() -> ForgeResult<()> {
        ...
        // read back (MessagePack + zstd) 直接反序列化为 imbl::HashMap<String, Arc<Node>>
        {
            ...
            let mut zr =
                ZipDocumentReader::new(file).map_err(|e| anyhow!(e))?;
            let meta_len =
                zr.read_all("meta.json").map_err(|e| anyhow!(e))?.len();
            let schema_len =
                zr.read_all("schema.xml").map_err(|e| anyhow!(e))?.len();
            // 也可用高层导入封装
            ...
            ) = import_zip_with_format(zip_path, SnapshotFormat::MsgPack, true)
                .map_err(|e| anyhow!(e))?;
