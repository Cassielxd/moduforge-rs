...
mod tests {
    ...
    async fn roundtrip_single_file() -> ForgeResult<()> {
        ...
        {
            let mut w = DocumentWriter::begin(file_path).map_err(|e| anyhow!(e))?;
            ...
            let meta_bytes = serde_json::to_vec(&meta_json).map_err(|e| anyhow!(e))?;
            w.add_segment(SegmentType::Meta, &meta_bytes).map_err(|e| anyhow!(e))?;
            w.add_segment(SegmentType::Schema, &schema_bytes).map_err(|e| anyhow!(e))?;

            // snapshot: MessagePack + zstd of Vec<ImHashMap<String, Arc<Node>>>
            // convert imbl::Vector -> Vec for compact serialization here
            let maps: Vec<ImHashMap<String, Arc<Node>>> = tree
                .nodes
                .iter()
                .cloned()
                .collect();
            let packed = rmp_serde::to_vec(&maps).map_err(|e| anyhow!(e))?;
            let compressed = zstd::stream::encode_all(&packed[..], 1).map_err(|e| anyhow!(e))?;
            w.add_segment(SegmentType::Snapshot, &compressed).map_err(|e| anyhow!(e))?;
        ...
        // read back
        {
            ...
            let meta_len = r.read_segment(SegmentType::Meta).map_err(|e| anyhow!(e))?.unwrap().len();
            let schema_len = r.read_segment(SegmentType::Schema).map_err(|e| anyhow!(e))?.unwrap().len();
            let snapshot_bytes = r.read_segment(SegmentType::Snapshot).map_err(|e| anyhow!(e))?.unwrap();
            let decoded = zstd::stream::decode_all(snapshot_bytes).map_err(|e| anyhow!(e))?;
            let maps: Vec<ImHashMap<String, Arc<Node>>> = rmp_serde::from_slice(&decoded).map_err(|e| anyhow!(e))?;
