...
pub trait RuntimeFactory {
    ...
    async fn create(options: RuntimeOptions) -> ForgeResult<Self::Runtime>;
...
mod tests {
    ...
    // 测试 RuntimeTrait 是否是对象安全的(可以用作 trait 对象)
    #[test]
    fn test_runtime_trait_object_safety() {
        // 如果这段代码能编译通过,说明 RuntimeTrait 是对象安全的
        fn _accept_runtime_trait(_runtime: &dyn RuntimeTrait) {}
    }
}
