...
impl BuildSnapshotConfig {
    ...
    /// 设置输出目录
    pub fn with_output_dir<P: AsRef<Path>>(
        mut self,
        dir: P,
    ) -> Self {
    ...
    /// 设置快照名称
    pub fn with_snapshot_name<S: AsRef<str>>(
        mut self,
        name: S,
    ) -> Self {
    ...
    /// 设置 Schema 目录
    pub fn with_schema_dir<P: AsRef<Path>>(
        mut self,
        dir: P,
    ) -> Self {
...
impl BuildTimeSnapshotGenerator {
    ...
    pub fn generate(&self) -> ForgeResult<CoreSnapshot> {
        ...
        // 2. 构建基础配置
        let mut config =
            ForgeConfig::for_environment(self.config.target_environment);
        ...
        for schema_path in xml_schemas {
            println!(
                "cargo:warning=添加 XML Schema: {}",
                schema_path.display()
            );
            builder = builder
                .add_xml_schema(schema_path.to_string_lossy().to_string());
        }
        ...
        // 5. 保存快照
        let snapshot_path =
            self.config.output_dir.join(&self.config.snapshot_name);
    ...
    fn discover_xml_schemas(&self) -> ForgeResult<Vec<PathBuf>> {
        ...
        let standard_locations =
            vec!["schemas", "schema", "src/schemas", "resources/schemas"];
        ...
        for (key, value) in &self.config.config_overrides {
            match key.as_str() {
                ...
                "performance.enable_monitoring" => {
                    config.performance.enable_monitoring =
...
mod tests {
    ...
    fn test_build_snapshot_config() {
        ...
        assert_eq!(
            config.config_overrides.get("processor.max_queue_size"),
            Some(&"1000".to_string())
        );
    }
