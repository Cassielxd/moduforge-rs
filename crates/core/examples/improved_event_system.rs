...
impl EventHandler<TestEvent> for FastHandler {
    async fn handle(&self, event: &TestEvent) -> ForgeResult<()> {
        // 模拟快速处理
...
impl EventHandler<TestEvent> for FlakyHandler {
    async fn handle(&self, event: &TestEvent) -> ForgeResult<()> {
        if rand::random::<f64>() < self.failure_rate {
            return Err(mf_core::error::error_utils::event_error(
                format!("FlakyHandler[{}] 模拟失败", self.id)
            ));
        }
...
async fn main() -> ForgeResult<()> {
    ...
    println!("  - 最大并发处理器: {}", event_bus.get_config().max_concurrent_handlers);
    ...
    println!("发送速率: {:.2} 事件/秒", event_count as f64 / send_duration.as_secs_f64());
...
mod tests {
    ...
    async fn test_event_processing_stats() -> ForgeResult<()> {
        ...
        assert!(stats.events_processed.load(std::sync::atomic::Ordering::Relaxed) >= 10);
        assert_eq!(stats.active_handlers.load(std::sync::atomic::Ordering::Relaxed), 1);
