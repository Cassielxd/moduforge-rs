...
async fn main() -> ForgeResult<()> {
    ...
    println!("  - 中间件超时: {}ms", custom_config.performance.middleware_timeout_ms);
    ...
    println!("\n=== 创建 ForgeRuntime ===");
    let runtime = mf_core::ForgeRuntime::create_with_config(options.clone(), custom_config.clone()).await?;
    
    println!("ForgeRuntime 配置:");
    let runtime_config = runtime.get_config();
    println!("  - 中间件超时: {}ms", runtime_config.performance.middleware_timeout_ms);
    ...
    let async_runtime = mf_core::ForgeAsyncRuntime::create_with_config(options, custom_config).await?;
    
    println!("ForgeAsyncRuntime 配置:");
    let async_config = async_runtime.get_config();
    println!("  - 任务接收超时: {}ms", async_config.performance.task_receive_timeout_ms);
    ...
    println!("  - 新的中间件超时: {}ms", updated_config.performance.middleware_timeout_ms);
    println!("  - 详细日志启用: {}", updated_config.performance.enable_detailed_logging);
    println!("  - 指标采样率: {}", updated_config.performance.metrics_sampling_rate);
...
mod tests {
    ...
    async fn test_config_inheritance() -> ForgeResult<()> {
        ...
        let async_runtime = mf_core::ForgeAsyncRuntime::create_with_config(options, config.clone()).await?;
    ...
    async fn test_config_update() -> ForgeResult<()> {
        // 测试配置更新
        let initial_config = ForgeConfig::for_environment(Environment::Development);
        let options = EditorOptionsBuilder::new()
            .content(Content::None)
            .extensions(vec![])
            .build();

        let mut async_runtime = mf_core::ForgeAsyncRuntime::create_with_config(options, initial_config).await?;
