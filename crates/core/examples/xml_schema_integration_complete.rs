...
async fn config_based_schema_example() -> ForgeResult<()> {
    ...
    println!("   ✅ 配置创建成功");
    println!("   - XML schema路径数量: {}", config.extension.xml_schema_paths.len());
...
async fn direct_xml_runtime_example() -> ForgeResult<()> {
    ...
    // 从XML内容创建同步Runtime
    let sync_runtime = ForgeRuntime::from_xml_content(xml_content, None, None).await?;
    ...
    // 从XML内容创建异步Runtime
    let _async_runtime = ForgeAsyncRuntime::from_xml_content(xml_content, None, None).await?;
...
async fn dynamic_extension_example() -> ForgeResult<()> {
    ...
    println!("   ✅ 动态添加XML扩展成功");
    println!("   - 最终节点数量: {}", extension_manager.get_schema().nodes.len());
    println!("   - 最终标记数量: {}", extension_manager.get_schema().marks.len());
...
async fn best_practices_example() -> ForgeResult<()> {
    ...
    let extension_manager = match ExtensionManager::from_xml_files(
        &config.extension.xml_schema_paths.iter().map(|s| s.as_str()).collect::<Vec<_>>()
    ) {
        Ok(manager) => manager,
        Err(e) => {
