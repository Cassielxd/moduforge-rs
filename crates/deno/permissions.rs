    ...
    RuntimePermissionDescriptorParser<TSys>
{
    ...
    fn resolve_cwd(&self) -> Result<PathBuf, PathResolveError> {
        self.sys.env_current_dir().map_err(PathResolveError::CwdResolve)
    }
    ...
    for RuntimePermissionDescriptorParser<TSys>
{
    ...
    fn parse_allow_run_descriptor(
        ...
        Ok(AllowRunDescriptor::parse(text, &self.resolve_cwd()?, &self.sys)?)
    }
...
mod test {
    ...
    fn test_handle_empty_value() {
        ...
        assert!(parser.parse_path_query(Cow::Borrowed(Path::new(""))).is_err());
