...
/// 用于将从开发者工具到检查器的连接进行代理的Websocket服务器。
    ...
    let (resp, fut) = match fastwebsockets::upgrade::upgrade(&mut req) {
        ...
        _ => {
            return http::Response::builder()
                .status(http::StatusCode::BAD_REQUEST)
                .body(Box::new(
                    Bytes::from("不是一个有效的Websocket请求").into(),
                ));
        },
    };

    // 生成一个任务，将等待websocket连接，然后在socket和检查器代理之间传递消息
    spawn(async move {
        let websocket = match fut.await {
            Ok(w) => w,
            Err(err) => {
                log::error!("检查器服务器升级到WS连接失败: {:?}", err);
                return;
            },
        };

        // '出站'通道携带发送到websocket的消息。
        let (outbound_tx, outbound_rx) = mpsc::unbounded();
        // '进站'通道携带从 websocket接收的消息。
    ...
    let mut deregister_inspector_handler = pin!(
        future::poll_fn(|cx| {
            inspector_map.borrow_mut().retain(|_, info| {
                info.deregister_rx.poll_unpin(cx) == Poll::Pending
            });
            Poll::<Never>::Pending
        })
        .fuse()
...
/// pump future负责在websocket和通道之间转发消息。
/// 当任一方断开连接时它会解决，忽略任何错误。
///
/// 该future将在warp WebSocket上发送和接收的消息代理到
/// UnboundedSender/UnboundedReceiver对。我们需要这些"无窗"通道端来绕过
/// Tokio的任务预算，当JsRuntimeInspector::poll_sessions()
/// 由于JavaScript执行被暂停而需要阻塞线程时，这会导致问题。
///
/// 这个方法有效是因为UnboundedSender/UnboundedReceiver在'futures'crate中实现，
/// 因此它们不能参与Tokio的协作任务让步。
...
impl InspectorInfo {
    ...
    pub fn get_websocket_debugger_url(
        &self,
        host: &str,
    ) -> String {
