...
fn op_create_worker(
    ...
    let worker_permissions =
        if let Some(child_permissions_arg) = args.permissions {
            parent_permissions
    ...
    let create_web_worker_cb =
        state.borrow::<CreateWebWorkerCbHolder>().clone();
    ...
    let maybe_worker_metadata = if let Some(data) = maybe_worker_metadata {
        ...
        Some(WorkerMetadata { buffer: data.data, transferables })
    } else {
        None
    };
    // Spawn it
    ...
    state.borrow_mut::<WorkersTable>().insert(worker_id, worker_thread);
...
async fn op_host_recv_ctrl(
    ...
    let maybe_event =
        worker_handle.get_control_event().or_cancel(cancel_handle).await;
    match maybe_event {
        Ok(Some(event)) => {
...
async fn op_host_recv_message(
    ...
    let ret =
        worker_handle.port.recv(state.clone()).or_cancel(cancel_handle).await;
    match ret {
        Ok(Ok(ret)) => {
            if ret.is_none() {
