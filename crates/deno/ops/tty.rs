...
fn op_set_raw(
        ...
        if stdin_state.reading {
            ...
            if original_mode & COOKED_MODE != 0 && !stdin_state.cancelled {
                ...
                let record = unsafe {
                    ...
                    *record.Event.KeyEvent_mut().uChar.UnicodeChar_mut() =
                        '\r' as u16;
                ...
                if unsafe {
                    winapi::um::wincon::WriteConsoleInputW(
                        handle, &record, 1, &mut 0,
                    )
                } == FALSE
                {
                    return Err(TtyError::Io(Error::last_os_error()));
    ...
    #[cfg(unix)]
    {
        ...
        let raw_fd =
            unsafe { std::os::fd::BorrowedFd::borrow_raw(handle_or_fd) };
        ...
        if is_raw {
            ...
            raw.control_chars
                [termios::SpecialCharacterIndices::VMIN as usize] = 1;
            raw.control_chars
                [termios::SpecialCharacterIndices::VTIME as usize] = 0;
