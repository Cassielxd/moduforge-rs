...
impl From<NotifyEvent> for FsEvent {
    fn from(e: NotifyEvent) -> Self {
        ...
        FsEvent { kind, paths: e.paths, flag }
    }
}
                ...
                if let Ok(event) = &res2 {
                    if paths.iter().any(|path| {
                        event.paths.iter().any(|event_path| {
                            same_file::is_same_file(event_path, path)
                                .unwrap_or(false)
...
async fn op_fs_events_poll(
    ...
    let resource =
        state.borrow().resource_table.get::<FsEventsResource>(rid)?;
    let mut receiver =
        RcRef::map(&resource, |r| &r.receiver).borrow_mut().await;
