            ...
            let mut futures = scripts
                ...
                .map(|script| {
                    ...
                    deno_core::unsync::spawn(async move {
                        ...
                        let (body, mime_type, res_url) = match script_url
                            .scheme()
                        {
                            "http" | "https" => {
                                let mut req = http::Request::new(
                                    deno_fetch::ReqBody::empty(),
                                );
                                *req.uri_mut() = script_url.as_str().parse()?;
                                ...
                                let resp = client
                                    .send(req)
                                    .await
                                    .map_err(FetchError::ClientSend)?;

                                if resp.status().is_client_error()
                                    || resp.status().is_server_error()
                                {
                                    return Err(
                                        SyncFetchError::InvalidStatusCode(
                                            resp.status(),
                                        ),
                                    );
                                }
                            ...
                            "blob" => {
                                ...
                                let mime_type =
                                    mime_type_essence(&blob.media_type);
                    ...
                    script_url.scheme().to_string(),
                        ...
                        if !loose_mime_checks {
                            ...
                            match mime_type.as_deref() {
                                ...
                                Some(mime_type) => {
                                    return Err(
                                        SyncFetchError::InvalidMimeType(
                                            mime_type.to_string(),
                                        ),
                                    );
                                },
                                None => {
                                    return Err(
                                        SyncFetchError::MissingMimeType,
                                    );
                                },
                            }
                        }
                        ...
                        let (text, _) =
                            encoding_rs::UTF_8.decode_with_bom_removal(&body);
