...
pub(crate) static SIGUSR2_RX: LazyLock<tokio::sync::watch::Receiver<()>> =
    LazyLock::new(|| {
        ...
        tokio::spawn(async move {
            let mut sigusr2 =
                deno_signals::signal_stream(libc::SIGUSR2).unwrap();
...
impl MainWorkerInternalHandle {
    ...
    pub fn post_event(
        ...
        (),
        mpsc::TrySendError<(ChannelRequest, oneshot::Sender<ChannelResponse>)>,
    > {
    ...
    pub fn terminate(&mut self) {
        ...
        let already_terminated =
            self.has_terminated.swap(true, Ordering::SeqCst);
...
pub struct SendableMainWorkerHandle {
    pub receiver:
        mpsc::Receiver<(ChannelRequest, oneshot::Sender<ChannelResponse>)>,
...
pub struct MainWorkerHandle {
    pub sender:
        mpsc::Sender<(ChannelRequest, oneshot::Sender<ChannelResponse>)>,
    termination_signal: Arc<AtomicBool>,
    has_terminated: Arc<AtomicBool>,
    terminate_waker: Arc<AtomicWaker>,
    isolate_handle: v8::IsolateHandle,
...
pub fn create_main_handles(
    ...
    let (ctrl_tx, ctrl_rx) =
        mpsc::channel::<(ChannelRequest, oneshot::Sender<ChannelResponse>)>(1);
...
) -> (Option<Rc<OpMetricsSummaryTracker>>, Option<OpMetricsFactoryFn>) {
...
impl MainWorker {
    pub async fn run(&mut self) -> Result<i32, CoreError> {
        ...
        loop {
            self.run_event_loop(/* wait for inspector */ false).await?;
    ...
    options.startup_snapshot.as_ref().expect("A user snapshot was not provided, even though 'only_snapshotted_js_sources' is used.");
        ...
        let mut js_runtime = if let Some(u) = options.unconfigured_runtime {
            ...
            common_runtime(CommonRuntimeOptions {
                ...
                enable_stack_trace_arg_in_ops: options
      ...
      (
        ...
        js_runtime
            .lazy_init_extensions(vec![
                ...
                deno_process::deno_process::args(
        ...
        let (
            ...
            let dispatch_load_event_fn =
                v8::Local::<v8::Function>::try_from(dispatch_load_event_fn)
                    .unwrap();
            ...
            let dispatch_unload_event_fn =
                v8::Local::<v8::Function>::try_from(dispatch_unload_event_fn)
                    .unwrap();
            ...
            let dispatch_process_exit_event_fn =
                v8::Local::<v8::Function>::try_from(
                    dispatch_process_exit_event_fn,
                )
                .unwrap();
            (
    ...
    pub async fn run_up_to_duration(
        ...
        match tokio::time::timeout(
            duration,
            self.js_runtime.run_event_loop(PollEventLoopOptions::default()),
        )
        .await
        {
            Ok(Ok(_)) => Ok(()),
    ...
    pub fn create_inspector_session(&mut self) -> LocalInspectorSession {
        ...
        self.js_runtime.inspector().borrow().create_local_session(
            InspectorSessionOptions { kind: InspectorSessionKind::Blocking },
        )
    }
    ...
    pub fn dispatch_process_exit_event(&mut self) -> Result<(), Box<JsError>> {
        let scope = &mut self.js_runtime.handle_scope();
        let tc_scope = &mut v8::TryCatch::new(scope);
        let dispatch_process_exit_event_fn = v8::Local::new(
            tc_scope,
            &self.dispatch_process_exit_event_fn_global,
        );
        let undefined = v8::undefined(tc_scope);
    ...
    pub fn dispatch_beforeunload_event(
        ...
        let dispatch_beforeunload_event_fn = v8::Local::new(
            tc_scope,
            &self.dispatch_beforeunload_event_fn_global,
        );
        let undefined = v8::undefined(tc_scope);
        let ret_val = dispatch_beforeunload_event_fn.call(
            tc_scope,
            undefined.into(),
            &[],
        );
...
impl UnconfiguredRuntime {
    ...
    >(
        ...
        UnconfiguredRuntime { module_loader, js_runtime }
    }
...
impl ModuleLoader for PlaceholderModuleLoader {
    fn resolve(
        ...
        self.0.borrow_mut().clone().unwrap().resolve(specifier, referrer, kind)
    }
    ...
    fn purge_and_prevent_code_cache(
        &self,
