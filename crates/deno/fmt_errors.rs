    ...
    fn fmt_element(
        ...
        match element {
            ...
            FunctionName | PromiseAll => {
                colors::italic_bold(s).to_string().into()
            },
        }
    }
}
...
fn find_recursive_cause(js_error: &JsError) -> Option<ErrorReference<'_>> {
    ...
    while let Some(cause) = &current_error.cause {
        ...
        if let Some(seen) = history.iter().find(|&el| cause.is_same_error(el)) {
            return Some(ErrorReference { from: current_error, to: seen });
        } else {
            current_error = cause;
        }
    }
...
fn format_js_error_inner(
    ...
    if let Some(aggregated) = &js_error.aggregated {
        let aggregated_message = format_aggregated_error(
            aggregated,
            circular.as_ref().map(|circular| circular.index).unwrap_or(0),
        );
            ...
            || msg.contains("require is not defined")
        {
            ...
            return vec![
                ...
                FixSuggestion::hint(cstr!(
                    "Use <u>import.meta.filename</> instead."
                )),
            ];
            ...
            return vec![
                ...
                FixSuggestion::hint(cstr!(
                    "Use <u>import.meta.dirname</> instead."
                )),
            ];
            ...
            return vec![
                FixSuggestion::info(
                    "new WebSocketStream() is an unstable API.",
                ),
                FixSuggestion::hint(
            ...
            return vec![
                FixSuggestion::info(
                    "Deno.UnsafeWindowSurface is an unstable API.",
                ),
                FixSuggestion::hint(
            ...
            .contains("client error (Connect): invalid peer certificate")
        {
...
pub fn format_js_error(js_error: &JsError) -> String {
    let circular = find_recursive_cause(js_error)
        .map(|reference| IndexedErrorReference { reference, index: 1 });
