            ...
            WorkerControlEvent::TerminalError(error) => {
                let value = match error.as_kind() {
                    CoreErrorKind::Js(js_error) => {
                        let frame = js_error.frames.iter().find(|f| {
                            match &f.file_name {
                                Some(s) => !s
                                    .trim_start_matches('[')
                                    .starts_with("ext:"),
...
/// 这是父线程用来与web工作器通信的句柄。
/// 它是从`SendableWebWorkerHandle`创建的，后者从创建它的工作器线程
/// 发送到父线程。这种分离的原因是句柄在线程间传输时首先需要是`Send`，
/// 然后在到达父线程时必须是`Clone`。没有大量的Arc<Mutex>和其他复杂
/// 结构，它不能同时满足这两个要求。
#[derive(Clone)]
...
impl WebWorkerHandle {
    /// 使用锁获取WorkerEvent
    /// 如果多个监听器尝试获取事件则返回错误
    #[allow(clippy::await_holding_refcell_ref)] // TODO(ry) 移除!
...
/// 此结构体是`Worker` Web API的实现
///
/// 每个`WebWorker`都是`MainWorker`或其他`WebWorker`的子项。
...
impl WebWorker {
    ...
    >(
        ...
        let mut extensions = vec![
            ...
            deno_process::deno_process::init(
    ...
    options.startup_snapshot.as_ref().expect("A user snapshot was not provided, even though 'only_snapshotted_js_sources' is used.");
        ...
        let mut js_runtime = JsRuntime::new(RuntimeOptions {
            ...
            validate_import_attributes_cb: Some(
                create_validate_import_attributes_callback(Arc::new(
                    AtomicBool::new(options.enable_raw_imports),
                )),
            ),
        ...
        let bootstrap_fn_global = {
            ...
            let bootstrap_ns: v8::Local<v8::Object> = global_obj
                ...
                .try_into()
                .unwrap();
            let main_runtime_str = v8::String::new_external_onebyte_static(
                scope,
                b"workerRuntime",
            )
            .unwrap();
    ...
    pub fn bootstrap(
        ...
        // WebWorkers can have empty string as name.
        {
            ...
            let mut worker_data: v8::Local<v8::Value> =
            ...
            let id_str: v8::Local<v8::Value> =
                v8::String::new(scope, &format!("{}", self.id)).unwrap().into();
    ...
    fn start_polling_for_messages(&mut self) {
        ...
        let fn_ =
            v8::Local::<v8::Function>::try_from(poll_for_messages).unwrap();
        let undefined = v8::undefined(scope);
    ...
    fn has_message_event_listener(&mut self) -> bool {
        ...
        let fn_ =
            v8::Local::<v8::Function>::try_from(has_message_event_listener)
                .unwrap();
        let undefined = v8::undefined(scope);
        // This call may return `None` if worker is terminated.
