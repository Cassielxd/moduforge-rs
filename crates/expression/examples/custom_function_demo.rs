...
fn main() -> anyhow::Result<()> {
    ...
    let result1 = isolate.run_standard_with_state("getMyStateCallCount()", my_state.clone())?;
    println!("  第一次调用结果: {}", result1);
    let result2 = isolate.run_standard_with_state("getMyStateCallCount()", my_state.clone())?;
    ...
    let result4 = isolate.run_standard_with_state("getMyStateCallCount()", my_state.clone())?;
    println!("  结果: {}", result4);
