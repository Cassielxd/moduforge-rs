use crate::functions::{
    ClosureFunction, DateMethod, DeprecatedFunction, FunctionKind,
    InternalFunction, MethodKind,
};
use crate::lexer::{
    Bracket, ComparisonOperator, Identifier, LogicalOperator, Operator,
    TokenKind,
};
...
impl<'arena, 'token_ref> Parser<'arena, 'token_ref, Unary> {
    ...
    fn expression_pair(&self) -> &'arena Node<'arena> {
        ...
        if let Some(TokenKind::Operator(Operator::Comparison(_))) =
            self.current_kind()
        {
            // Skips
        } else {
        ...
        match self.current_kind() {
            Some(TokenKind::Operator(Operator::Comparison(comparison))) => {
                self.next();
                let right_node =
                    self.binary_expression(0, ParserContext::Global);
                left_node = self.node(
                    Node::Binary {
                        left: left_node,
    ...
    fn binary_expression(
        ...
        while let TokenKind::Operator(operator) = &token.kind {
            ...
            let node_right = match op.associativity {
                Associativity::Left => self.binary_expression(
                    op.precedence + 1,
                    ParserContext::Global,
                ),
                _ => {
                    self.binary_expression(op.precedence, ParserContext::Global)
                },
            };
    ...
    fn unary_expression(&self) -> &'arena Node<'arena> {
                ...
                == TokenKind::Identifier(Identifier::CallbackReference)
        {
            ...
            let (n, _) =
                self.with_postfix(node, &|c| self.binary_expression(0, c));
            return n;
        }
        ...
        if let Some(interval_node) =
            self.interval(&|c| self.binary_expression(0, c))
        {
        ...
        if token.kind == TokenKind::Bracket(Bracket::LeftParenthesis) {
            ...
            let (n, _) =
                self.with_postfix(expr, &|c| self.binary_expression(0, c));
            return n;
        }
