            ...
            match op {
                        ...
                        (Array(a), Number(n)) => {
                            ...
                            self.push(
                                arr.get(n.to_usize().ok_or_else(|| {
                                    OpcodeErr {
                                        opcode: "Fetch".into(),
                                        message: "Failed to convert to usize"
                                            .into(),
                                    }
                                })?)
                                .cloned()
                        ...
                        (Number(a), Number(b)) => {
                            let result = a
                                ...
                                .ok_or_else(|| OpcodeErr {
                                    ...
                                    message: "Failed to calculate exponent"
                                        .into(),
                                })?;
                    ...
                    let parts = arr
                        ...
                        .map(|(i, var)| match var {
                            ...
                            _ => Err(OpcodeErr {
                                opcode: "Join".into(),
                                message: format!(
                                    "Unexpected type in array on index {i}"
                                ),
                            }),
                        })
                ...
                Opcode::AssignedObjectStep => {
                    ...
                    let Some(assigned_object) = &assigned_object else {
                        return Err(OpcodeErr {
                            opcode: "AssignedObjectStep".into(),
                            message: "Assigned object scope must be set"
                                .to_owned(),
                        });
                    };

                    let Some(new_env) =
                        env.dot_insert_detached(key.as_ref(), value.clone())
                    else {
                        return Err(OpcodeErr {
                            opcode: "AssignedObjectStep".into(),
