...
mod tests {
    ...
    fn test_edge_cases() {
        ...
        if let Some(matched_state) = matched {
            let fill_result_no_end =
                matched_state.fill(&empty_content, false, &schema);
            println!(
                "fill result (to_end=false): {:?}",
                fill_result_no_end.is_some()
            );
        }
    }
    ...
    fn test_block_choice_problem() {
        ...
        if let Some(content_match) = &block_type.content_match {
            ...
            let result =
                block_type.create_and_fill(None, None, vec![], None, &schema);
    ...
    fn test_sequence_with_existing_nodes() {
        ...
        let (main_node, child_nodes) = result.into_parts();
        println!(
            "Main node: type={}, content={:?}",
            main_node.r#type, main_node.content
        );
        println!("Child nodes count: {}", child_nodes.len());
    ...
    fn test_table_creation_step_by_step() {
        ...
        if child_nodes.is_empty() {
            ...
            for (i, child) in child_nodes.iter().enumerate() {
                ...
                for (j, grandchild) in grandchildren.iter().enumerate() {
                    let (gc_node, great_grandchildren) =
                        grandchild.clone().into_parts();
                    println!(
                        "        Grandchild {}: type={}, id={}",
                        j + 1,
                        gc_node.r#type,
                    ...
                    println!(
                        "          Great-grandchildren count: {}",
                        great_grandchildren.len()
                    );
                }
            }
        }
    ...
    fn test_sequence_table_problem() {
                    ...
                    for (j, grandchild) in grandchildren.iter().enumerate() {
                        ...
                        if gc_node.r#type == "tablerow" {
                            if great_grandchildren.is_empty() {
                                ...
                                let tablerow_type =
                                    schema.nodes.get("tablerow").unwrap();
                                if let Some(tr_content_match) =
                                    &tablerow_type.content_match
                                {
                                    ...
                                    let empty_content: Vec<Node> = vec![];
                                    let tr_matched = tr_content_match
                                        .match_fragment(
                                            &empty_content,
                                            &schema,
                                        );
                                    if let Some(tr_matched_state) = tr_matched {
                                        println!(
                                            "            tablerow match_fragment 成功"
                                        );
                                        println!(
                                            "            tablerow matched state valid_end: {}",
                                            tr_matched_state.valid_end
                                        );

                                        let tr_fill_result = tr_matched_state
                                            .fill(
                                                &empty_content,
                                                true,
                                                &schema,
                                            );
                                        if let Some(tr_needed_types) =
                                            tr_fill_result
                                        {
                                            println!(
                                                "            tablerow 需要的类型数量: {}",
                                            ...
                                            println!(
                                                "            ❌ tablerow fill 返回 None"
                                            );
                                        }
                                    } else {
                                        println!(
                                            "            ❌ tablerow match_fragment 返回 None"
                                        );
                                    }
                                } else {
                                    println!(
                                        "            ❌ tablerow 没有 content_match"
                                    );
                                }
                            } else {
                                println!(
                                    "          ✅ tablerow 创建了 {} 个 tablecell",
                                    great_grandchildren.len()
                                );
                                for (k, ggc) in
                                    great_grandchildren.iter().enumerate()
                                {
                                    let (ggc_node, _) =
                                        ggc.clone().into_parts();
                                    println!(
                                        "            曾孙节点 {}: type={}, id={}",
                                        k + 1,
                                        ggc_node.r#type,
        ...
        println!("\n=== 对比：单独创建 Table 节点 ===");
        let table_type = schema.nodes.get("table").unwrap();
        let standalone_table =
            table_type.create_and_fill(None, None, vec![], None, &schema);
        ...
        println!("\n=== 额外调试：单独创建 tablerow 节点 ===");
        let tablerow_type = schema.nodes.get("tablerow").unwrap();
        let standalone_tablerow =
            tablerow_type.create_and_fill(None, None, vec![], None, &schema);
