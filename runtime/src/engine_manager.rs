...
impl EngineManager {
    pub fn create(root: Option<PathBuf>) -> EngineManager {
        let loader = FilesystemLoader::new(FilesystemLoaderOptions {
            keep_in_memory: true,
            root: root.unwrap_or(current_dir().unwrap()).as_path().to_string_lossy().to_string(),
        });
        EngineManager { engine: Arc::new(DecisionEngine::default().with_loader(Arc::new(loader))) }
    }
    pub async fn evaluate<K, T>(
        &self,
        key: K,
        context: Variable,
    ) -> Result<T, Error>
