...
impl DocumentCache {
    ...
    pub fn get(
        ...
        // 2. 尝试L2读取
        if let Ok(v) = self.l2.get(format!("{}{}", key.doc_id.clone(), key.version)) {
            // 3. 回填L1
    ...
    fn load_from_storage(
        ...
        // 从全量快照+增量日志重构文档
        let base_path = self.storage_option.storage_path.join(key.clone().doc_id.as_str()).join("snapshot");
    ...
    pub fn put(
        ...
        self.l2.put(format!("{}{}", key.doc_id.clone(), state.version), state.doc());
    }
}
