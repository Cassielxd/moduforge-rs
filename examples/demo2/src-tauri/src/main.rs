...
fn register_window(
    ...
    println!("窗口已注册: {window_id} (父窗口: {parent_id:?})");
}
...
async fn create_module_window(
    ...
    println!("创建模块窗口: {module_key} - {title} - URL: {url}");
    ...
    let webview_url =
        if url.starts_with("http://") || url.starts_with("https://") {
            ...
            let parsed_url = url.parse().map_err(|e| {
                let error_msg = format!("URL解析失败: {e} - URL: {url}");
                println!("{error_msg}");
                error_msg
            })?;
...
async fn create_child_window(
    ...
    println!(
        "创建子窗口: {window_id} - {title} - URL: {url} - Modal: {modal:?}"
    );
    ...
    println!("处理后的父窗口ID: {parent_id} (模态: {is_modal})");
    ...
    child_window.on_window_event(move |event| {
        if let WindowEvent::CloseRequested { .. } = event {
            println!("子窗口 {window_id_clone} 即将关闭");
...
async fn broadcast_to_children(
    ...
    println!("广播完成，成功发送到 {success_count} 个子窗口");
    Ok(())
}
