...
fn main() -> anyhow::Result<()> {
    ...
    Isolate::register_custom_function(
        ...
        |args, state_opt| {
            ...
            if let Some(state) = state_opt {
                // 检查插件是否存在（简化版本）
                let has_plugin = state.plugins().iter().any(|p| &p.key == plugin_name);
    ...
    // 测试2: 传递State运行
    println!("\n2. 传递State运行表达式:");
    let result2 = isolate.run_standard_with_state("getStateInfo()", state.clone())?;
    println!("   结果: {}", result2);

    // 测试3: 获取文档标题
    println!("\n3. 获取文档标题:");
    let result3 = isolate.run_standard_with_state("getDocumentTitle()", state.clone())?;
    println!("   结果: {}", result3);

    // 测试4: 检查插件是否存在
    println!("\n4. 检查插件是否存在:");
    let result4 = isolate.run_standard_with_state("checkPlugin('example-plugin')", state.clone())?;
    println!("   结果: {}", result4);
