...
fn demo_basic_compilation() {
    ...
    let result = evaluate_expression(expression, context.clone().into()).unwrap();
    println!("计算结果: {:?}", result);
...
fn demo_data_types() {
    ...
    let test_cases = vec![
        ("user.name + \" is \" + string(user.age) + \" years old\"", "字符串拼接"),
        ("user.age >= 18", "布尔运算"),
...
fn demo_intervals_and_conditions() {
    ...
    let test_expressions = vec![
        ...
        ("student.score in (rules.passing_score..rules.excellent_score)", "良好区间判断（开区间）"),
        ("student.age in [18..65) and student.score >= rules.passing_score", "复合条件"),
    ];
