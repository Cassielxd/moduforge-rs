...
impl PartialEq for Schema {
  fn eq(
    ...
    self.spec == other.spec && self.top_node_type == other.top_node_type && self.nodes == other.nodes && self.marks == other.marks
  }
}
impl Eq for Schema {}
impl Schema {
  ...
  pub fn compile(instance_spec: SchemaSpec) -> Result<Schema, Box<dyn Error>> {
    ...
    schema.top_node_type = schema
      .nodes
      .get(&schema.spec.top_node.clone().unwrap_or_else(|| "doc".to_string()))
      .cloned();
