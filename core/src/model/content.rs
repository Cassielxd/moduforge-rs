...
impl ContentMatch {
    ...
    pub fn fill(&self, after: &Vec<Node>, to_end: bool, schema: &Schema) -> Option<Vec<Node>> {
...
impl TokenStream {
    pub fn new(string: String, node_types: HashMap<String, NodeType>) -> Self {
...
fn resolve_name(stream: &TokenStream, name: &str) -> Vec<NodeType> {
...
pub fn null_from(nfa: &[Vec<Rc<RefCell<Edge>>>], node: usize) -> Vec<usize> {
    let mut result = Vec::new();
    fn scan(nfa: &[Vec<Rc<RefCell<Edge>>>], node: usize, result: &mut Vec<usize>) {
        let edges = &nfa[node];
...
fn edge(
    ...
    let edge = Rc::new(RefCell::new(Edge {
        term,
        to: Option::from(to.unwrap_or(0)),
    }));
    nfa[from].push(edge.clone());
    edge.clone()
}
fn connect(edges: &mut [Rc<RefCell<Edge>>], to: usize) {
    for edge in edges {
