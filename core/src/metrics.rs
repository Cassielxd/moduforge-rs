...
// 插件管理器 指标
/// 插件管理器创建耗时（秒）
pub const EXTENSION_MANAGER_CREATION_DURATION_SECONDS: &str = "core.extension_manager.creation.duration.seconds";
/// 已加载扩展总数
...
pub fn task_processed(status: &str) {
    counter!(TASKS_PROCESSED_TOTAL, "status" => status.to_string()).increment(1);
}
...
pub fn command_executed(name: &str) {
    counter!(COMMANDS_EXECUTED_TOTAL, "command_name" => name.to_string()).increment(1);
}
...
pub fn middleware_execution_duration(duration: std::time::Duration, mw_type: &str, mw_name: &str) {
    histogram!(
...
pub fn history_operation(op: &str) {
    counter!(HISTORY_OPERATIONS_TOTAL, "operation" => op.to_string()).increment(1);
}

pub fn event_emitted(event_type: &str) {
    counter!(EVENTS_EMITTED_TOTAL, "event_type" => event_type.to_string()).increment(1);
}

pub fn extension_manager_creation_duration(duration: std::time::Duration) {
    histogram!(EXTENSION_MANAGER_CREATION_DURATION_SECONDS).record(duration.as_secs_f64());
}
